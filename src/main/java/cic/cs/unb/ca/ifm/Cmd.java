...
public class Cmd {
    ...
    private static void readPcapDir(File inputPath, String outPath, long flowTimeout, long activityTimeout) {
        ...
        int increment = pcapFiles.length / threadCount;
        for(int t = 0; t < threadCount; t++){
           threads[t] = new Thread(new worker(t * increment, (t+1) * increment, pcapFiles, outPath, flowTimeout, activityTimeout));
           threads[t].start();
        }
    ...
    private static void readPcapFile(String inputFile, String outPath, long flowTimeout, long activityTimeout) {
        ...
        int nTotal = 0;
        int nDiscarded = 0;
        while (packetReader.hasNextPacket()) {
